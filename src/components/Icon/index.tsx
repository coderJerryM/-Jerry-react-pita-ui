...
function isValidCustomScriptUrl(url: string): boolean {
  return Boolean(typeof url === 'string' && url.length && !customCache.has(url))
}
...
export default function Icon(props: IIconProps) {
  ...
  if (icon && type) {
    ...
    // 自定义的iconfont
    loadUrl(url)
    const iconCls = classnames('iconfont', prefix ? `${prefix}-${icon}` : `iconfont-${icon}`)
...
Icon.propTypes = {
  ...
  url: function (props: IIconProps, propName: string, componentName: string) {
    if (props.url && props.custom) {
      const regUrl = /^(\/\/)?((?:[\w-]+\.)+[a-z0-9]+)((?:\/[^/?#]*)+)?(\?[^#]+)?(#.+)?$/i
      if (!regUrl.test(props.url)) {
        return new Error(
          'Invalid iconfont Url supplied to' + ' `' + componentName + '`. Validation failed.'
        )
      }
    }
  }
